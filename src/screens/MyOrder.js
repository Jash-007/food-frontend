...
export default function MyOrder() {
    ...
    const fetchMyOrder = async () => {
        await fetch("https://food-backend-tjsa.onrender.com/api/auth/myOrderData", {
            // credentials: 'include',
    ...
    for (let i = 0; i < orderData.order_data.length; i++) {
        const element = orderData.order_data[i];
        const order_date = element[0].Order_date; // order date
        for (let j = 1; j < element.length; i++) {
            const order = element[i];
        }
    }
    return <div> ok </div>
    // return (
